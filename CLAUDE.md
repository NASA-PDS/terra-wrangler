...
- **`python_requires = >= 3.9`, not `>= 3.13`.** The template defaults to the org's current standard Python (3.13), but tf-sheriff's validator is pure-stdlib (argparse, pathlib, re, json) with nothing 3.13-specific, and it needs to run inside many other repos' CI environments that may pin an older interpreter. Revisit if that stops being true.
