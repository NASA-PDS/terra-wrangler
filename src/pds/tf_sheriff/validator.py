...
def read(path: Path) -> str:
    """Read a file's text, tolerating encoding errors and missing files."""
    try:
...
def find_deployable_modules(root: Path):
    """Return every directory under root that contains a main.tf, sorted."""
    modules = []
...
def is_reusable_module(module_dir: Path, root: Path) -> bool:
    """Return True if module_dir looks like a reusable/library module, not a deployable root."""
    rel = module_dir.relative_to(root)
    return "modules" in rel.parts or "examples" in rel.parts


def load_ignores(root: Path):
    """Load .tfvalidate-ignore entries as a list of (check_id, path_substring_or_None)."""
    ignores = []
...
def is_ignored(ignores, check_id, path_str):
    """Return True if check_id is ignored for path_str per the loaded .tfvalidate-ignore entries."""
...
class Report:
    """Accumulates check results for a single module or the repo as a whole."""

    def __init__(self, label, kind):
        """Initialize an empty report for the given label/kind (e.g. a module path and its type)."""
        self.label = label
        self.kind = kind
    ...
    def findings(self, severity=None, status="fail"):
        """Return checks matching the given status (default: failures), optionally filtered by severity."""
...
def check_module(module_dir: Path, root: Path, ignores) -> Report:
    """Run all module-level Must-Have/Should-Have checks against module_dir and return a Report."""
...
def check_repo_level(root: Path, ignores) -> Report:
    """Run repo-level checks (gitignore coverage, CI wiring, linter config) and return a Report."""
    ...
    workflows = list(workflows_dir.glob("*.y*ml")) if workflows_dir.exists() else []
    ...
    if tf_workflows:
        ...
        detail = f"found {[w.name for w in tf_workflows]}, but apply step looks disabled/absent" if not live else ""
        add("S1", "a Terraform CI workflow exists and runs `terraform apply` (not just commented out)",
            "warn", live, detail=detail)
    else:
    ...
    if precommit.exists():
        ...
        mentions_linter_lines = [
            line for line in ptext.splitlines() if "tflint" in line.lower() or "checkov" in line.lower()
        ]
        precommit_active = bool(re.search(r'tflint|checkov', ptext, re.I)) and not all(
            line.strip().startswith("#") for line in mentions_linter_lines
        )
...
def render_text(module_reports, repo_report, strict):
    """Render the human-readable text report and return it as a single string."""
    lines = []
    ...
    lines.append(
        f"SUMMARY: {errors} Must-Have error(s), {warnings} Should-Have warning(s), "
        f"{skipped} skipped via .tfvalidate-ignore."
    )
    if errors:
        ...
        extra = f" {warnings} warning(s) to address when convenient." if warnings else ""
        lines.append(f"RESULT: PASS — no Must-Have failures.{extra}")
    return "\n".join(lines)


def render_github(module_reports, repo_report):
    """Render GitHub Actions ::error::/::warning:: annotation lines for every failing check."""
    lines = []
...
def to_json(module_reports, repo_report):
    """Build the machine-readable JSON representation of all reports."""

    def dump(r):
        return {"label": r.label, "kind": r.kind,
                "checks": [{"id": c[0], "title": c[1], "severity": c[2], "status": c[3], "detail": c[4]}
                           for c in r.checks]}
...
def main():
    """Parse arguments, run all checks, print the requested report format, and return the exit code."""
